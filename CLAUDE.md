...
- release-please uses `release-type: simple` (config in `release-please-config.json`, manifest in `.release-please-manifest.json`)
- `update-debian-changelog` job runs `dch` on the release PR branch to keep `debian/changelog` in sync
